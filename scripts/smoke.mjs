...
try {
  ...
  check("homepage has the agreed sections and real anchors", () => {
    ...
    assert(JSON.stringify(result.sections) === JSON.stringify([
      "hero", "products", "platform", "standards", "journey", "engineering", "resources", "start-building",
  ...
  check("engineering coverage matches the generated API catalog", () => {
    const catalog = JSON.parse(readFileSync(new URL("../public/search-index.json", import.meta.url), "utf8"));
    ...
    assert(result.operations === expected, `${result.operations} operations shown; catalog has ${expected}`);
  ...
  check("journey code tabs are keyboard-accessible and local only", () => {
    ...
    ab("press", "ArrowRight");
    ab("wait", "--fn", 'document.getElementById("journey-request-tab-1").getAttribute("aria-selected") === "true"');
    ab("press", "End");
    ab("wait", "--fn", 'document.getElementById("journey-request-tab-2").getAttribute("aria-selected") === "true"');
    const result = evalJson(`JSON.stringify({
    ...
    assert(result.code.includes("http://localhost:4001/v1/auth/me"), "sample does not use the local service");
    assert(!result.code.includes("api.id.qeet.in"), "sample targets production");
    ab("press", "Home");
    ab("click", ".journey-code .codetabs__copy");
    ab("wait", "--fn", 'document.querySelector(".journey-code .codetabs__copy").classList.contains("is-copied")');
  ...
  check("homepage fits phone, tablet, landscape, and desktop viewports", () => {
    const viewports = [
      ...
      [600, 900], [640, 960], [768, 1024], [820, 1180], [844, 390],
      [1024, 768], [1199, 800], [1200, 800], [1279, 800], [1280, 800],
    ...
    for (const [width, height] of viewports) {
      ...
      assert(result.pageWidth <= width && result.overflow.length === 0,
        `${width}px overflow: page=${result.pageWidth}, elements=${result.overflow}`);
      assert(result.headlineLines === 2 && result.headlineFits, `${width}px hero headline does not fit two lines`);
      assert(result.ecosystemLines === 2 && result.ecosystemFits, `${width}px ecosystem heading does not fit two lines`);
      assert(JSON.stringify(result.ecosystemText) === JSON.stringify([
        "Every Qeet product.", "One developer experience.",
      ]), "ecosystem heading text or word spacing changed");
      assert(result.smallTargets.length === 0, `${width}px undersized hero controls: ${result.smallTargets}`);
    }
  ...
  check("mobile menu scrolls, contains focus, and unlocks on resize", () => {
    ...
    ab("set", "viewport", "1440", "950");
    ab("wait", "--fn", 'document.getElementById("nav-toggle").getAttribute("aria-expanded") === "false"');
